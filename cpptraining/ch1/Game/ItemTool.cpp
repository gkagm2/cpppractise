...
void AddItem(_tagItem* pWeaponStore, _tagItem* pArmorStore) {
	...
	// 인자로 무기상점에서 사용할 아이템 배열과 방어구 상점에서 사용할 아이템 배열을 따로 넣어줌.
	// 아이템을 추가할때에도 추가할 상점을 선택하고 아이템의 난이도를 선택했다.
	// 그러므로 복잡하기 때문에 여기에서 추가할 아이템의 메모리 주소를 얻어와서
	// 거기에 추가해주면 쉽게 처리가 가능하다.
	if (iStore == SM_WEAPON) {
...
void Output(_tagItem* pWeapon, _tagItem* pArmor) {
	...
	for (int i = 0; i < STORE_WEAPON_MAX; ++i) {
		...
		cout << "설명 : " << pWeapon[i].strDesc << endl;
		cout << endl;
	}
